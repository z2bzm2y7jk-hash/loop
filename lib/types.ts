...
export type CourseTee={name:string;location:string;courseRating:number;slopeRating:number;pars:number[];strokeIndexes:number[];source:'demo'|'manual'};
export type Round={started?:boolean;id:string;course:string;tee?:CourseTee;holes:number;players:Player[];games:Game[];config:Config;results:Hole[];presses:Press[];paid:string[];date:string};
