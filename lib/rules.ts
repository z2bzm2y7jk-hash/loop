import {Config,Game,Hole,Round,handicaps} from './types';
export type Rules={
...
export function scoresFor(r:Round,h:Hole,index:number,net:boolean){return h.scores.map((s,i)=>s-(net?Math.floor(r.players[i].handicap/18)+(handicaps[index]<=r.players[i].handicap%18?1:0):0))}
